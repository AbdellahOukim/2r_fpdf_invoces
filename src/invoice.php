...
class Invoice extends PDF_MC_Table
{
    ...
    {
        $position = $this->data['header']['alingement'] == 'L' ? $this->pagePaddingX + 2 : ($this->pageContainerWidth - $width);
        $this->Image($path, $position, ($this->headerHeight / 2 - ($height / 2.5)), $width, $height);
    }

    protected function companyInfo()
    {
        $y =  $this->headerHeight / 2 - 5;
        $x = $this->data['header']['alingement'] == 'L' ? $this->pageContainerWidth / 2 : $this->pagePaddingX + 2;
        $this->SetFont('Arial', 'B', 8);

        foreach ($this->data['header']['company_infos'] as $line) {
            $this->SetXY($x, $y);
            $this->MultiCell($this->pageContainerWidth / 2, 5, $line, 0, "C");
            $y = $this->GetY();
        }
    }
    ...
    protected function Table()
    {
        ...
        foreach ($this->data['table']['tbody'] as $dt) {
            ...
            if ($counter == count($this->data['table']['tbody'])) {
                ...
                $clacCondition = $this->hasConditions || $this->hasEnLettre || $this->hasValidite ? $this->calcConditionsSize()['size'] : 0;
                $clacTotal = $this->hasTotal ? $this->calcTotal() : 0;

                $this->globalY = $clacTotal > $clacCondition  ?   $height - $clacTotal : $height - $clacCondition;
                ...
                if ($credentials['y2'] + $clacCondition > $positionTable || $credentials['y2'] + $clacTotal  > $positionTable) {
                    // * here 
                    $x1 = $x;
    ...
    {
        $this->pagePaddingY = $options['print']['paddingY'] ?? 5;
        ...
        $this->type = $options['document']['type'] ?? 'D';

        if (isset($options['document']['type']) && $this->type != 'D') {
            $this->hasEnLettre = $options['document']['hasEnLettre'] ?? false;
            $this->hasConditions = $options['document']['hasConditions'] ?? false;
            $this->hasValidite = $options['document']['hasValidite'] ?? false;
            $this->hasTotal = $options['document']['hasTotal'] ?? false;
        }
    }
    ...
    public function generateInvoice()
    {
        ...
        if ($this->type != "D")
            $this->DocumentInfo();
        $this->Table();
        if ($this->hasConditions || $this->hasEnLettre || $this->hasValidite)
            $this->Conditions();
        $this->Output();
    }
}
